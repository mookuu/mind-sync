...
- **feat**: 权限分离重构 — per-user 同步设置（DB V7 迁移）、搜索/库按用户 synced 源过滤、路由守卫角色拦截
...
- **feat**: 同步素材 3 区布局 + 默认库置顶勾选 + 全量重建 modal
...
- **fix**: 添加全局库后 toast 提示消息（`addCustomPath` 增加 `toast.success`/`toast.error`）
- **fix**: 成员页面全局知识库全部可见且可自主勾选（移除 `!isAdmin` 过滤/禁用，改用 `sync_key` 匹配）
- **fix**: 管理员个人知识库可勾选同步、共享/取消共享、删除（三区恢复 checkbox + 共享/删除按钮）
- **fix**: 共享知识库 checkbox `sync_key` 匹配修复（`p.id` → `p.sync_key || p.id`）
- **refactor**: 删除同步控制页面「知识库状态」section（模板/逻辑/样式/`loadMissingFiles`）
- **refactor**: 同步范围 radio 从「全部/自定义/仅共享」简化为「全部同步/自定义」
- **refactor**: 全局重命名「素材管理」→「同步素材」（路由/侧边栏/页面标题/全部文档）
- **feat**: 全局库共享机制 — 管理员可 🔒/🔓 控制全局库对角色可见性，`sources.yaml` 支持 `shared` 字段
- **feat**: `SyncSourcesAdmin.vue` 素材管理表格 — 全局源显示「全局」标签，所有源统一共享/删除操作
- **fix**: 非管理员「我的知识库」移除冗余用户信息标签（`👤 moku 当前用户(5)`）
- **fix**: 全局库/共享库空内容时隐藏标签
- **refactor**: 同步素材 radio button 模式 — 全部同步时库列表隐藏，自定义时展开
- **feat**: 管理员素材管理页面 — 分页列表所有用户知识库（含全局默认源 owner=admin）、拥有者/状态下拉筛选、共享/删除操作按钮、操作记入审计日志并通知库拥有者
