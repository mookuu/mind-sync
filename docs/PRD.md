...
├──────────────────────────────────────────────────────────────┤
│ API 层: FastAPI — 5 routers (auth/admin/user/knowledge/content) │
├──────────────────────────────────────────────────────────────┤
...
| ID | 功能 | 说明 | 版本 |
|----|------|------|------|
| F12 | 同步素材 UI | 全局知识库 / 我的知识库 / 共享知识库 三区布局 | 2026-07 |
...
| F32 | 管理员素材管理 | 分页列表所有用户知识库，筛选/共享/删除操作，审计日志+owner 通知 | 2026-07 |
...
| 角色 | 标识 | 能力 |
|------|------|------|
| **管理员 (admin)** | `admin` | 全部功能：同步、同步素材、用户管理、系统管理、Wiki 编辑、所有操作记录 |
| **成员 (member)** | `member` | 搜索、文档库、知识查询、同步（仅自己的源）、同步素材（个人库）、操作记录（仅自己） |
...
同步素材页面提供两个 radio button：**全部同步** / **自定义**。

| 角色 | 全部同步范围 | 自定义范围 |
|------|-------------|-----------|
| admin | 所有已共享的全局库 + 所有个人库 + 已勾选共享库 | 手动勾选全局库、个人库、共享库 |
| member | 所有已共享的全局库 + 其他用户共享库 | 手动勾选已共享的全局库、个人库、共享库 |

> **全局库共享**：管理员可通过全局库行的 🔒/🔓 按钮控制全局库对角色用户的可见性。未共享的全局库在"全部同步"和"自定义"中均不可见。
...
| US-03 | member 无法读取他人私有 Wiki | P0 | ✅ done |
| US-04 | API Key 审计追溯到用户 | P0 | ✅ done |
| US-05 | 环境变量 AUTH_USERS 识别 member | P0 | ✅ done |
| US-06 | main.py 拆分为 routers/ (151行) | P1 | ✅ done |
| US-07 | Swagger 补充 Pydantic response_model | P1 | ✅ done |
| US-08 | 删除重复工作流文档 | P2 | ✅ done |
| US-09 | ROADMAP 反映代码现状 | P2 | ✅ done |
| US-10 | API Key 绑定用户 | P3 | ✅ done |
| US-11 | 权限测试补充 | P4 | ✅ done |
...
### Phase 0+1 全部完成（2026-07）

| 阶段 | 内容 | 状态 |
|------|------|------|
| P0 | US-03 Wiki 隔离（safe_wiki_path role 判断） | ✅ |
| P0 | US-04 API Key 审计绑定 | ✅ |
| P1 | US-06 main.py 拆分（1607→151行，5 router） | ✅ |
| P1 | US-07 Pydantic response_model 挂载 | ✅ |
| P1 | browse_documents + list_category_stats 权限过滤 | ✅ |
| P2 | US-09 ROADMAP + 文档体系标准化 | ✅ |
| P3 | US-10 API Key 绑定用户 | ✅ |
| P4 | US-11 权限边界测试补充 | ✅ |
| P4 | S10-01~04 Toast/Skeleton/搜索增强/流式输出 | ✅ |
